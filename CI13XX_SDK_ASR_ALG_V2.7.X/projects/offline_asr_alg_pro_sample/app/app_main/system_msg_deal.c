...
static void servo_pwm_init(void)
{
    //scu_set_pwm_clk_div(220); // 220MHz / 220 = 1MHz
    // 1. 使能PA端口时钟
...
void servo_set_angle(uint8_t angle)
{
    ...
    // 按舵机标准：0°=500μs, 180°=2500μs
       uint32_t min_pulse = 500;   // 0°对应500us
...
void enter_wakeup_deal(uint32_t exit_wakup_ms, cmd_handle_t cmd_handle )
{
    ...
    mprintf("#####in enter_wakeup wakeup cmd_id:%d\n", wakeup_cmd_id);
    xSemaphoreTake(WakeupMutex, portMAX_DELAY);
	...
	vpr_run_one_recognition(wakeup_cmd_id);
	#endif
}
