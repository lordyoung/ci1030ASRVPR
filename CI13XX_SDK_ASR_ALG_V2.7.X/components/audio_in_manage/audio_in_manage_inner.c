...
void audio_in_manage_inner_task(void *p)
{
    ...
    // IIS_SLAVE模式：CI13322做主提供SCLK/LRCK，CI1303做从接收
    // 不需要PAD_CLK修复和TX使能（时钟由CI13322驱动）
    ...
    for (;;)
    {
...

        {
            ...
            if (dbg_cnt < 5)
            {
                int16_t *dbg16 = (int16_t *)data_addr;
                uint32_t *dbg32 = (uint32_t *)data_addr;
                volatile uint32_t iis_int0 = *(volatile uint32_t *)(HAL_IIS0_BASE + 0x00);
                volatile uint32_t iis_rx_ctrl0 = *(volatile uint32_t *)(HAL_IIS0_BASE + 0x20);
                volatile uint32_t iis_dma_chk = *(volatile uint32_t *)(HAL_IIS0_BASE + 0x34);
                volatile uint32_t iis_clk_chk = *(volatile uint32_t *)(HAL_IIS0_BASE + 0x40);
                // int0 key bits: bit23=rr0_err bit9=fifo_under bit8=fifo_over bit1=lrck bit0=sck
                mprintf("[AUDIO_DBG] frm=%d int16=%d,%d,%d,%d uint32=0x%08x int0=0x%08x(err=%d,un=%d,ov=%d) rx=0x%x dma_chk=0x%x clk_chk=0x%x\n",
                    dbg_cnt,
                    dbg16[0],dbg16[1],dbg16[2],dbg16[3],dbg32[0],
                    iis_int0,
                    (iis_int0>>23)&1, (iis_int0>>9)&1, (iis_int0>>8)&1,
                    iis_rx_ctrl0, iis_dma_chk, iis_clk_chk);
                dbg_cnt++;
            }
        }
