...
static int config_iis_dma(codec_info_t * p_codec_info, io_direction_t io_dir)
{
    ...
    if(IIS_MASTER == p_iis_info->iis_mode_sel)
    {
        if (CODEC_INPUT == io_dir)   //录音时钟
        {
            if (IIS0 == p_iis_info->IISx)
            {
                // IIS0 MASTER RX: use SRC1→MCLK1 (shared with IIS1)
                // MCLK1 is proven to drive PAD pins for internal CODEC
    ...
    mprintf("[IIS_DIAG] IISx=%d dev_sel=%d dir=%d mode=%d\n",
            p_iis_info->IISx, device_select, io_dir, p_iis_info->iis_mode_sel);
    mprintf("[IIS_DIAG] clk_cfg=%d clk_mode=%d mclk_src=%d mclk=%d\n",
            cap_iis_clk_str.clk_cfg, cap_iis_clk_str.clk_mode,
            cap_iis_clk_str.mclk_cfg.src, cap_iis_clk_str.mclk_cfg.mclk);
    mprintf("[IIS_DIAG] fs=%d sck_lrck=%d div=%d src_cfg_div=%d\n",
            cap_iis_clk_str.mclk_cfg.fs, cap_iis_clk_str.mclk_cfg.sck_lrck,
            (int)div_f, cap_iis_clk_str.src_cfg.source_div);

    if (IIS0 == p_iis_info->IISx && CODEC_INPUT == io_dir)
    {
        mprintf("[IIS_DIAG] IIS0 MASTER RX: SRC2->MCLK2, model=%d, clk_mode=%d\n",
                cap_iis_clk_str.model_sel, cap_iis_clk_str.clk_mode);
    }
    ...
    if (CODEC_INPUT == io_dir)
    {
        ...
        if (IIS0 == p_iis_info->IISx)
        {
            mprintf("[IIS_DIAG] enabling RX clock detection\n");
            iis_rx_chk(p_iis_info->IISx, p_iis_info->rx_cha, ENABLE);
        }
        ...
        if (IIS0 == p_iis_info->IISx)
        {
            volatile uint32_t int0 = *(volatile uint32_t *)((uint32_t)p_iis_info->IISx + 0x00);
            volatile uint32_t chk_rx = *(volatile uint32_t *)((uint32_t)p_iis_info->IISx + 0x40 + p_iis_info->rx_cha * 4);
            volatile uint32_t reg_src0 = *(volatile uint32_t *)(HAL_SCU_BASE + 0xC0);
            volatile uint32_t reg_mclk0 = *(volatile uint32_t *)(HAL_SCU_BASE + 0xD0);
            volatile uint32_t reg_iis0_clk = *(volatile uint32_t *)(HAL_SCU_BASE + 0xE0);
            volatile uint32_t reg_pad_clk = *(volatile uint32_t *)(HAL_SCU_BASE + 0xF0);
            mprintf("[IIS_DIAG] MASTER RX config complete:\n"
                    "  int0=0x%x chk_rx=0x%x\n"
                    "  SRC0=0x%x MCLK0=0x%x IIS0_CLK=0x%x PAD_CLK=0x%x\n",
                    int0, chk_rx,
                    reg_src0, reg_mclk0, reg_iis0_clk, reg_pad_clk);

            // pad_config_for_iis已通过dpmu_set_io_direction设置PA3/PA5为OUTPUT
            // SCU寄存器由iis_clk_config+scu_iis_pad_clk_config配置为MASTER+OUTPUT
            mprintf("[IIS_DIAG] IIS0 MASTER RX ready - clocks driven on PA3(LRCLK)/PA5(SCLK)\n");
        }
    }
    else
    {
...
int cm_start_codec(int codec_index, io_direction_t io_dir)
{
    ...
    if (CODEC_INPUT == io_dir)
    {
        ...
        mprintf("[IIS_TRACE] cm_start idx=%d IIS=%d\n", codec_index, IISx);
