...
typedef enum
{
    VPR_REC_FAILED = 19,           // 识别失败
    VPR_REG_SUCCESSED       = 12, // 注册成功
    VPR_REG_FAILED          = 17, // 注册失败
    VPR_REG_RESAMPLE        = 13, // 继续重复录入声纹
    VPR_REC_SUCCESSED       = 1, // 识别成功
    VPR_REG_RESAMPLE_FAILED = 14, //重复录入失败(注册一个模板可能需要录入多次)。
    VPR_REG_REC_NULL       = 15, //没有注册或者识别结果0 
}vpr_reg_rst_t;
 ...
 * @return 0:没有错误; 非0:错误号。
 */
extern int vpr_run_one_recognition(uint16_t wakeup_cmd_id);
