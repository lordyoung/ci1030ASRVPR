 ...
 * @return 0:没有错误; 非0:错误号。
 */
//修改函数参数，将wakeup_cmd_id传入函数，方便在识别成功后通过回调函数将识别结果传回给应用层
int vpr_run_one_recognition(uint16_t wakeup_cmd_id)
{
    ...
    case VPR_STATUS_REC:
        ...
        if (vpr_info.vpt_number)
        {
            ...
            if (vpr_info.callback_func)
            {
                vpr_info.callback_func(vpr_reg_rec_null, 0);
            }
            mprintf("####recognition rec switch vpr_info.vpt_number=0, cmd_id:%d\n", wakeup_cmd_id);
    ...
    case VPR_STATUS_IDLE:
            if (vpr_info.callback_func)
                ...
                mprintf("####recognition rec switch VPR_STATUS_IDLE, cmd_id:%d\n", wakeup_cmd_id);
        break;
    ...
    switch(rst)
    {
        ...
        case vpr_reg_rec_null:
            ...
            while (prompt_is_playing()) {
                ...
                // 直接使用语音ID播放，避免命令查找开销
                //int play_result = prompt_play_by_voice_id(14, default_play_done_callback, true);
                //mprintf("play voice 14 result: %d\n", play_result);
                //mprintf("voice print recognition reg rec null, VPR_REG_REC_NULL is %d\n", VPR_REG_REC_NULL);
                        
            prompt_play_by_cmd_id(15,-1,default_play_done_callback,true);
            //prompt_play_by_voice_id(14, default_play_done_callback, true);
            mprintf("voice print recognition reg rec null,VPR_REG_REC_NULL is %d\n", VPR_REG_REC_NULL);
            while (prompt_is_playing()) {
                vTaskDelay(pdMS_TO_TICKS(10));  // 每10ms检查一次,不然会被后面的命令截胡覆盖本次播放            }    
            }
        }
            break;
        default:
