...
void pad_config_for_uart(UART_TypeDef *UARTx)
{
    ...
    else if (UARTx == UART2)
    {
        ...
        dpmu_set_io_reuse(PC2,THIRD_FUNCTION);
...
{
    // IIS0引脚复用为IIS功能
    // CI1303(MASTER) → CI13322(SLAVE): PA2=SDI, PA3=LRCLK, PA5=SCLK, PA6=MCLK
    // 不设置dpmu_set_io_direction，让IIS外设自行控制引脚方向
...
const cm_codec_hw_info_t host_mic_hw_info = 
{
    ...
    .IICx = IIC_NULL,
    .output_iis.IISx = IIS1,                      // 播放仍用IIS1+内部CODEC
