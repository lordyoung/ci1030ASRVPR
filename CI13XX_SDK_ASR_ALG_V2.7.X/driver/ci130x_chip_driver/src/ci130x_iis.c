...
void iis_wait_load_ctrl(uint32_t iis_base)
{
    ...
    uint32_t iis_id = (iis_base == HAL_IIS0_BASE) ? 0 : (iis_base == HAL_IIS1_BASE) ? 1 : 2;
    ...
    while(iis->iis_load_ctrl.val)
    {
        ...
        if(iis_timeout == 0)
        {
            ...
            if(retry_cnt <= 5)
            {
                mprintf("iis_load_ctrl timeout! IIS%d base=0x%x load_ctrl=0x%x\n", iis_id, iis_base, iis->iis_load_ctrl.val);
            }
            if(retry_cnt >= 10)
            {
                mprintf("iis_load_ctrl GIVEUP IIS%d base=0x%x load_ctrl=0x%x\n", iis_id, iis_base, iis->iis_load_ctrl.val);
                iis->iis_load_ctrl.val = 0;
                mprintf("iis_load_ctrl force-clear IIS%d\n", iis_id);
                return;
            }
        }
    }
    if(retry_cnt > 0)
    {
        mprintf("iis_load_ctrl OK IIS%d retry=%d\n", iis_id, retry_cnt);
    }
}
...
void iis_rx_enable(uint32_t iis_base,iis_rx_channal_t cha,FunctionalState cmd)
{
    iis_register_p iis = (iis_register_p)iis_base;
    uint32_t iis_id = (iis_base == HAL_IIS0_BASE) ? 0 : (iis_base == HAL_IIS1_BASE) ? 1 : 2;
    mprintf("[IIS_TRACE] rx_en enter IIS%d cmd=%d load=0x%x\n", iis_id, cmd, iis->iis_load_ctrl.val);
    iis_wait_load_ctrl(iis_base);
    if ((cmd == ENABLE && iis->iis_rx_ctrl[cha].rx_en) ||
        (cmd == DISABLE && !iis->iis_rx_ctrl[cha].rx_en))
    {
        mprintf("[IIS_TRACE] rx_en skip IIS%d already in state\n", iis_id);
        return;
    }
    if(cmd == ENABLE)
    {
    ...
    iis_wait_load_ctrl(iis_base);
    mprintf("[IIS_TRACE] rx_en exit IIS%d\n", iis_id);
}
...
void iis_rx_config(uint32_t iis_base,iis_rx_config_p rx_cfg)
{
    ...
    mprintf("[IIS_TRACE] rx_cfg enter IIS%d\n", iis_id);
    iis_wait_load_ctrl(iis_base);
    ...
    iis_wait_load_ctrl(iis_base);
    mprintf("[IIS_TRACE] rx_cfg exit IIS%d\n", iis_id);
}
